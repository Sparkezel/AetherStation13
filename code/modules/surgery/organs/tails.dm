...
// Lazy copy paste from felinid code
/obj/item/organ/tail/fox
	name = "fox tail"
	desc = "A severed fox tail. It smells like fried tofu."
	tail_type = "Fox"

/obj/item/organ/tail/fox/Insert(mob/living/carbon/human/tail_owner, special = FALSE, drop_if_replaced = TRUE)
	..()
	if(istype(tail_owner))
		var/default_part = tail_owner.dna.species.mutant_bodyparts["tail_human"]
		if(!default_part || default_part == "None")
...
/obj/item/organ/tail/fox/Remove(mob/living/carbon/human/tail_owner, special = FALSE)
	..()
	if(istype(tail_owner))
		tail_owner.dna.species.mutant_bodyparts -= "tail_human"
		color = tail_owner.hair_color
